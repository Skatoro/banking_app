...
import React, {FC, InputHTMLAttributes, memo, PropsWithChildren, useState} from "react";
...
export const CardSettingItem: FC<Props> = memo(({text, Icon, DropdownContent, onClick, disabledButton, children}) => {
    ...
    return (
        <div className={'relative '}>
            <button
                ...
                className={`flex items-center py-3 px-2 w-full hover:bg-secondary dark:hover:bg-bang1 border-x-2 border-t-2 ${roundStyle}
                ${active || disabled ? 'bg-secondary dark:bg-bang1 border-stone-500' : 'border-transparent'}`}
            >
                ...
                {DropdownContent &&
                    <ChevronDown className={active ? styles.rotateIconActivate : styles.rotateIconDisable}
                    />}
            </button>

            {DropdownContent && <>
                <div
                    className={`absolute left-0 bg-secondary dark:bg-bang1 z-30 overflow-hidden w-full rounded-b-3xl 
